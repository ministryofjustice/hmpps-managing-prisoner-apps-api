...
ALTER TABLE if exists app
    ADD CONSTRAINT app_status_check
        CHECK ((status)::text = ANY
               ((ARRAY ['PENDING'::character varying,'NEW'::character varying, 'IN_PROGRESS'::character varying, 'APPROVED'::character varying, 'REJECTED'::character varying, 'DECLINED'::character varying])::text[]));
...
ALTER TABLE if exists history ADD CONSTRAINT history_activity_check
    CHECK ((activity)::text = ANY
           ((ARRAY
               ...
               'APP_IN_PROGRESS':: character varying,
