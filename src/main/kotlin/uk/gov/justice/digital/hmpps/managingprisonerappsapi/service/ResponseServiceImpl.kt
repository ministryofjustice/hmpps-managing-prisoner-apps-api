...
class ResponseServiceImpl(
  ...
  override fun addResponse(prisonerId: String, appId: UUID, staffId: String, response: AppDecisionRequestDto): AppDecisionResponseDto<Any> {
    ...
    app.requests.forEach { request ->
      ...
      if (response.appliesTo.contains(request.get("id") as UUID)) {
  ...
  override fun getResponseById(prisonerId: String, appId: UUID, staffId: String, createdBy: Boolean, responseId: UUID): AppDecisionResponseDto<Any> {
    ...
    if (createdBy) {
      val establishment = establishmentService.getEstablishmentById(staff.establishmentId).orElseThrow {
        ApiException("Staff establishment ${staff.establishmentId} not added in establishment record", HttpStatus.BAD_REQUEST)
      }
      staffDto = StaffDto(
        staff.username,
        staff.userId,
    ...
    app.requests.forEach {  request ->
      if (request["responseId"] == responseId) {
        appliesTo.add(request["id"] as UUID)
      }
    }
  ...
  private fun convertResponseToAppDecisionResponse(prisonerId: String, requestIds: List<UUID>, appId: UUID,  response: Response): AppDecisionResponseDto<Any> {
